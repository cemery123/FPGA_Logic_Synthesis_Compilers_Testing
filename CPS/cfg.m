classdef cfg
    ...
    properties(Constant = true)
        ...
        NUM_TESTS = 50000;                       % Number of random models to generate (and use in differntial testing) 生成模型数，原150
        
        NUM_BLOCKS = [100 120];                 % 块数范围250-300
        
        COMPARE_SIM_RESULTS = false;         % Compare simulation results obtained by logging signals ("Compare" phases) 比较仿真结果
        
        CLOSE_MODEL = true;                    % Close models after experiment  运行后关闭模型
        PAUSE_BETWEEN_FIX_ERROR_STEPS = false;
        STOP_IF_ERROR = false;                  % Stop the script when meet the first simulation error 遇到错误报错停止
        STOP_IF_DTC_ERROR = false;           % Data type conversion from typesmart analysis 数据类型转换错误时停止
...
%          phases of CyFuzz
        ...
        LOAD_RNG_STATE = false;                  % -false-Set this `true` if we want to create NEW models each time the script is run. Set to `false` if generating same models at each run of the script is desired. For first time running in a new computer set to false, as this will fail first time if set to true.
        ...
        MAX_HIERARCHY_LEVELS =1;               % Minimum value is 1 indicating a flat model with no hierarchy.层次结构为多少
        ...
        SL_BLOCKLIBS = {
           struct('name', 'Logic and Bit Operations', 'is_blk', false, 'num', 0) % 0.1
           %struct('name', 'Discrete', 'is_blk', false, 'num', 0.05)
%             struct('name', 'Continuous', 'is_blk', false,  'num', 0.1)
             struct('name', 'Math Operations', 'is_blk', false,  'num', 0.92) % 0.78
%             struct('name', 'Logic and Bit Operations', 'is_blk', false,  'num', 0.08)
            struct('name', 'Sinks', 'is_blk', false, 'num', 0.02) % 0.02
            struct('name', 'Sources', 'is_blk', false, 'num', 0.02) % 0.02
            %struct('name', 'hdlsllib/Sources/Constant', 'is_blk', true, 'num', 0.06)
            struct('name', 'hdlsllib/Ports & Subsystems/Subsystem', 'is_blk', true, 'num', 0.02) % 0.04
           struct('name', 'hdlsllib/Signal Routing/Switch', 'is_blk', true, 'num', 0.02) % 0.04
        ...
        SL_BLOCKS_BLACKLIST = {
            ...
            'hdlsllib/Source/Counter Free-Running'
            'hdlsllib/Source/Counter Limited'
            'hdlsllib/Source/Enumerated Constant'
            'hdlsllib/Source/Ground'
            'hdlsllib/Source/Repeating Sequence Stair'
